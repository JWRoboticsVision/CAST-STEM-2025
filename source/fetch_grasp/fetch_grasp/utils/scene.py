...
import fetch_grasp.utils.grid as utils_grid
...
class WorldService:
    def __init__(self):
        ...
        self.property_service = rospy.ServiceProxy(self.world_service_name, GetWorldProperties)
...
class ObjectService:
    def __init__(
        ...
        """
        self.models_base_path = str(models_base_path)
        ...
        self.delete_object_client = rospy.ServiceProxy("/gazebo/delete_model", DeleteModel)

        rospy.wait_for_service(spawn_object_service_name)
        self.add_object_client = rospy.ServiceProxy("/gazebo/spawn_sdf_model", SpawnModel)
    ...
    def delete_object(self, obj_name):
        ...
        if not response.success:
            print(f"{obj_name}: failed to delete! check name, existence of object in current scene")
...
class SceneMaker:
    ...
    def _create_table_grid(self):
        ...
        _verts_org = utils_grid.generate_grid(grid_size[0], grid_size[1], self._table_pos, 1)
    ...
    def _process_object_models(self):
        self.model_meshes = {ycbid: utils_grid.load_mesh(self.models_path, ycbid) for ycbid in self.model_names}
        if self._stable_poses_f is not None:
            with open(self._stable_poses_f, "rb") as f:
                selected_poses = pickle.load(f)
            self.model_stable_poses = {ycbid: selected_poses[ycbid] for ycbid in self.model_names}
            print("Loaded stable poses for all objects from file")
        else:
            self.model_stable_poses = {
                ycbid: trimesh.poses.compute_stable_poses(self.model_meshes[ycbid], threshold=0.02)
...
def make_args():
    parser = argparse.ArgumentParser(description="Generate grid and spawn objects", add_help=True)
    parser.add_argument(
        "--ws_dir",
        type="str",
