...
from fetch_grasp.utils.commons import read_data_from_json

add_path(PROJECT_ROOT / "third-party/NIDS-Net")
...
class CropResizePad:
    ...
    def __call__(self, images, boxes):
        ...
        for image, box, scale in zip(images, boxes, scale_factor):
            ...
            assert image.shape[1] == image.shape[2], logging.info(f"image {image.shape} is not square after padding")
            image = F.interpolate(image.unsqueeze(0), scale_factor=self.target_h / image.shape[1])[0]
...
class CustomDINOv2(pl.LightningModule):
    def __init__(
        ...
        self.img_resize = T.Compose([T.Resize((patch_size, patch_size), interpolation=T.InterpolationMode.BICUBIC)])
    ...
    def compute_features(self, images, masks):
        if images.shape[0] > self.chunk_size:
            ...
            features = (grid * masks.permute(0, 2, 3, 1)).sum(dim=(1, 2)) / masks.sum(dim=(1, 2, 3)).unsqueeze(-1)
            appearance_feat = F.normalize(
                (grid * masks.permute(0, 2, 3, 1)).view(len(images), mask_size * mask_size, -1), dim=-1
            )
            # use class token
    ...
    def forward_cls_token(self, image_np, proposals):
        processed_rgbs, processed_masks = self.process_rgb_proposals(image_np, proposals["masks"], proposals["boxes"])
    ...
    def forward_by_chunk_v2(self, processed_rgbs, masks):
        ...
        features = BatchedData(batch_size=self.chunk_size)
        for idx_batch in range(len(batch_rgbs)):
            feats = self.compute_masked_patch_feature(batch_rgbs[idx_batch], batch_masks[idx_batch])
            features.cat(feats)
    ...
    for category_id in unique_category_ids:
        ...
        if len(category_boxes) > 0:
            nms_indices = torchvision.ops.batched_nms(
                category_boxes, category_scores, category_ids[category_mask], iou_threshold
            )
...
class NIDS:
    ...
    def get_template_features(self, template_image, mask):
        ...
        proposals = dict()
        proposals["masks"] = fg_mask.to(torch.float32)  # to N x H x W, torch.float32 type as the output of fastSAM
        proposals["boxes"] = bounding_box_tensor

        query_FFA_decriptors, query_appe_descriptors, query_cls_descriptors = self.descriptor_model(
            template_image, proposals
        )
    ...
    def step(self, image_np, THRESHOLD_OBJECT_SCORE=0.60, visualize=False):
        ...
        proposals["masks"] = masks.squeeze(1).to(
            torch.float32
        )  # to N x H x W, torch.float32 type as the output of fastSAM
